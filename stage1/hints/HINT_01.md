...
LLM에는 **컨텍스트 창(context window)** 이 있고, 넣는 만큼 **비용이 발생**합니다.
...
컨텍스트 창이 1M 토큰이라 데이터가 물리적으로 들어갈 수도 있습니다.
하지만 매 요청마다 전체 데이터를 보내면:
1. **비용 폭발** — 데이터가 수십만 토큰이면 요청당 수십 원, 하루 수만 원
2. **응답 품질 저하** — 관련 없는 문서가 섞이면 LLM이 엉뚱한 답을 생성
