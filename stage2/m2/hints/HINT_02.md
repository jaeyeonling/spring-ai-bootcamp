...
## ReRanker가 역효과를 낼 수도 있다

작은 데이터셋(수십 건)에서는 코사인 유사도만으로 이미 충분한 정확도가 나옵니다.
이 경우 후보 20개를 뽑으면 전체 데이터의 대부분이 후보에 포함되고,
LLM 채점이 오히려 노이즈를 만듭니다.

지금 환경(3,100건)에서는 사정이 다릅니다.
후보 20개는 전체의 0.6%에 불과하므로 ReRanker가 제 역할을 할 가능성이 높습니다.
**가정하지 말고 직접 측정하세요** — Baseline과 비교해서 정확도 변화를 기록하세요.
